...
void MCL::updatePos() {
	...
	//std::cerr<<"U<";
	float x, sx2, y, sy2,st2, sxa2, sya2, xa, ya, t;
	...
	for (int i = 0; i < NUMPARTICLES; i++) {
		...
		//std::cerr<<"Pos = ("<<cx<<", "<<cy<<") ["<<particles[i].p<<"]"<<std::endl;
		//t = normalizePi(t + angle * particles[i].p);
	...
	for (int i = 0; i < NUMPARTICLES; i++) {
		...
		st2 = st2 + particles[i].p * (normalizePi(yaw - t) * normalizePi(yaw - t));
	 ...
	 */
	//std::cerr<<"Pos = ("<<pose.pose.position.x<<", "<<pose.pose.position.y<<") ["<<sx2<<", "<<sy2<<", "<<st2<<"]"<<std::endl;
	for (int i = 0; i < 36; i++)
...
void MCL::updateObservation2(std::string obs, std::string real) {
	...
	tf::StampedTransform O2R;

	try {
		tfL.lookupTransform(obs, "base_link", 
				ros::Time::now() - ros::Duration(0.2), O2R);
		/*tfL.lookupTransform("/world", "/base_footprint",
				ros::Time::now() - ros::Duration(0.2), W2R);*/
...
std::cerr<<"Pos = ("<<dista2obs<<", "<<dista2ideal<<std::endl;
...
void MCL::reseed() {
	...
	for (int i = NUMPARTICLES - 20; i < NUMPARTICLES; i++) {
